...
void ImprovedWater::create(ImprovedTerrain &r_terrain) {
    ...
    m_waternoise.SetFrequency(0.1f);

    m_waterlevel = r_terrain.getWaterlevel();
    for(int x=0;x<r_terrain.getSize();x++)
    {
        for(int z=0;z<r_terrain.getSize();z++)
        {
            if(r_terrain.getHeightOfTerrain(x,z)<m_waterlevel&&!r_terrain.isBorder(x,z))
            {
...
WaterTile ImprovedWater::generateTile(int x, int z) {
    ...
    buf.x_pos_base =x;
    buf.z_pos_base =z;
    buf.y_pos_leftup = m_waternoise.GetNoise(static_cast<float>(x - size), static_cast<float>(z - size));
    buf.y_pos_leftbottom = m_waternoise.GetNoise(static_cast<float>(x - size), static_cast<float>(z + size));
    buf.y_pos_rightup = m_waternoise.GetNoise(static_cast<float>(x + size), static_cast<float>(z - size));
    buf.y_pos_rightbottom = m_waternoise.GetNoise(static_cast<float>(x + size), static_cast<float>(z + size));
    return buf;
}
...
void ImprovedWater::convertWatertileToFloatAndAddToVBOdata(WaterTile tile) {
    ...
    vert_nw.posy_own = m_waternoise.GetNoise(tile.x_pos_base - size, tile.z_pos_base - size);
    vert_nw.posx = tile.x_pos_base;
    vert_nw.posz = tile.z_pos_base;
    vert_nw.vertex = nw;

    vert_ne.posy_own = m_waternoise.GetNoise(tile.x_pos_base + size, tile.z_pos_base - size);
    vert_ne.posx = tile.x_pos_base;
    vert_ne.posz = tile.z_pos_base;
    vert_ne.vertex = ne;

    vert_sw.posy_own = m_waternoise.GetNoise(tile.x_pos_base - size, tile.z_pos_base + size);
    vert_sw.posx = tile.x_pos_base;
    vert_sw.posz = tile.z_pos_base;
    vert_sw.vertex = sw;

    vert_se.posy_own = m_waternoise.GetNoise(tile.x_pos_base + size, tile.z_pos_base + size);
